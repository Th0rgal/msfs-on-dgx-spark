...
By default, `90-remote-dgx-stable-check.sh` exits early when all DGX candidates resolve to Tailscale endpoints and local `tailscaled` is unavailable (`DGX_FAST_FAIL_ON_UNREACHABLE_TAILSCALE=1`), but it now bypasses that fail-fast path when direct TCP reachability is detected on a candidate endpoint/port. Set `DGX_FAST_FAIL_ON_UNREACHABLE_TAILSCALE=0` only when you intentionally want full SSH timeout probing in every case.
When local Tailscale status is available, known-offline Tailscale candidates are skipped by default (`DGX_SKIP_OFFLINE_TAILSCALE_CANDIDATES=1`) and listed with `last seen` metadata; set `DGX_SKIP_OFFLINE_TAILSCALE_CANDIDATES=0` if you want to probe those endpoints regardless.
For containerized/non-systemd runners, use `BOOTSTRAP_LOCAL_TAILSCALE=1` to start a userspace `tailscaled` and route SSH through its SOCKS proxy. For unattended bootstrap, pass `LOCAL_TAILSCALE_AUTHKEY` or `LOCAL_TAILSCALE_AUTHKEY_FILE`; otherwise run `tailscale --socket /tmp/msfs-on-dgx-spark-tailscaled.sock login` once and rerun the check. `LOCAL_TAILSCALE_AUTHKEY_FILE` is fail-closed by default (must exist, be non-empty, and mode `600`; override with `REQUIRE_LOCAL_TAILSCALE_AUTHKEY_FILE_PERMS=0`). Userspace auth state now persists by default at `${XDG_STATE_HOME:-$HOME/.local/state}/msfs-on-dgx-spark/tailscaled.state`; set `LOCAL_TAILSCALE_STATE=...` to customize location. Interactive login is opt-in (`LOCAL_TAILSCALE_INTERACTIVE_LOGIN=0` by default) so headless runs fail quickly with a login URL instead of waiting for interactive login timeout; set `LOCAL_TAILSCALE_INTERACTIVE_LOGIN=1` and optionally raise `LOCAL_TAILSCALE_LOGIN_TIMEOUT_SECONDS` when you intentionally want in-script interactive auth. In interactive mode, the script now re-runs `tailscale up` after login to continue bootstrap without requiring a second invocation.
...
If retries stop on exit code `7` after an earlier authenticated attempt, enable `AUTO_REAUTH_ON_AUTH_FAILURE=1`; the retry runner will now invoke `58-ensure-steam-auth.sh` between attempts before treating auth loss as fatal.
