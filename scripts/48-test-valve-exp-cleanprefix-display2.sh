#!/usr/bin/env bash
# Clean-prefix Valve Experimental cycle on NVIDIA display :2.
set -euo pipefail
...
RUN_LOG="$OUT_DIR/valve-exp-cleanprefix-display2-cycle-$TS.log"
SHOT="$OUT_DIR/valve-exp-cleanprefix-display2-$TS.png"
PROTON_LOG="$OUT_DIR/steam-$MSFS_APPID.log"

mkdir -p "$OUT_DIR"

{
  echo "== $(date -u +%FT%TZ) valve-exp clean-prefix display2 cycle =="
  ...
  echo "[4/8] Ensure Proton Experimental is a real directory and pristine"
  if [ -L "$EXP_DIR" ]; then
  ...
  echo "[5/8] Clean compat prefix and set minimal launch options"
  rm -rf "$COMPAT_DIR"
  mkdir -p "$COMPAT_DIR"
  ...
  echo "[6/8] Launch native Steam under FEX on display $DISPLAY_NUM"
  ...
  grep -nE "StartSession: appID $MSFS_APPID|Command prefix|waitforexitandrun|Proton Experimental|Steam Linux Runtime" "$STEAM_DIR/logs/compat_log.txt" | tail -n 160 || true
  grep -nE "pressure-vessel-wrap|Internal error: .*vulkan|Proton: Upgrading prefix|invalid version|Adding process|Removing process|Impossible to create DirectX12|D3D12CreateDevice|vkd3d_create_vk_device" "$STEAM_DIR/logs/console-linux.txt" | tail -n 260 || true
