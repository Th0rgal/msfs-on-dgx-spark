...
echo "  Baseline retry recovery: ${RECOVER_BETWEEN_ATTEMPTS}"
echo "  Strict retry recovery: ${STRICT_RECOVER_BETWEEN_ATTEMPTS}"
...
if MSFS_APPID="$MSFS_APPID" \
  ...
  WAIT_SECONDS="$WAIT_SECONDS" \
  ATTEMPT_PAUSE_SECONDS="$ATTEMPT_PAUSE_SECONDS" \
  RECOVER_BETWEEN_ATTEMPTS="$STRICT_RECOVER_BETWEEN_ATTEMPTS" \
  RECOVER_ON_EXIT_CODES="$RECOVER_ON_EXIT_CODES" \
