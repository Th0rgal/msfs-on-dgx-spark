...
`90-remote-dgx-stable-check.sh` supports `DGX_PORT=<port>` for non-default SSH ports and `DGX_PORT_CANDIDATES=22,2222,...` to probe multiple ports automatically before failing; it also auto-discovers Tailscale IPv4 endpoints from the candidate host list by default (`DGX_DISCOVER_TAILSCALE_IPS=1`, disable with `0`).
