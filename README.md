...
`90-remote-dgx-stable-check.sh` supports `DGX_PORT=<port>` for non-default SSH ports and `DGX_PORT_CANDIDATES=22,2222,...` to probe multiple ports automatically before failing; candidates are normalized/deduplicated before probing. It also accepts `DGX_ENDPOINT_CANDIDATES=host1:22,host2:2222,...` when you need explicit host+port pair ordering, and auto-discovers Tailscale IPv4 endpoints from the candidate host list by default (`DGX_DISCOVER_TAILSCALE_IPS=1`, disable with `0`). For routed access, use `DGX_SSH_PROXY_JUMP=user@jump-host` or `DGX_SSH_PROXY_COMMAND='ssh -W %h:%p jump-host'` (mutually exclusive), and optional `DGX_SSH_EXTRA_OPTS_CSV='IdentityFile=/path/key,UserKnownHostsFile=/dev/null'` for additional `ssh -o` options. When all targets are Tailscale-only and local `tailscaled` is unavailable, the script still fail-fasts by default (`DGX_FAST_FAIL_ON_UNREACHABLE_TAILSCALE=1`) unless direct TCP reachability is detected; in that case it proceeds with SSH probing automatically. Set `DGX_FAST_FAIL_ON_UNREACHABLE_TAILSCALE=0` to always force full SSH probing. When local Tailscale status is available, known-offline Tailscale candidates are skipped by default (`DGX_SKIP_OFFLINE_TAILSCALE_CANDIDATES=1`, set `0` to force probing anyway), and errors include peer `last seen` diagnostics.
For non-systemd/local-container runners, set `BOOTSTRAP_LOCAL_TAILSCALE=1` to start userspace `tailscaled` and auto-wire SSH through `nc -x <LOCAL_TAILSCALE_SOCKS5_ADDR> -X 5`; provide `LOCAL_TAILSCALE_AUTHKEY` (or `LOCAL_TAILSCALE_AUTHKEY_FILE`) for unattended login, or run `tailscale --socket <LOCAL_TAILSCALE_SOCKET> login` once interactively. `LOCAL_TAILSCALE_AUTHKEY_FILE` is fail-closed (`must exist`, non-empty, and mode `600` by default; override with `REQUIRE_LOCAL_TAILSCALE_AUTHKEY_FILE_PERMS=0`). Userspace state now defaults to a persistent path (`LOCAL_TAILSCALE_STATE=${XDG_STATE_HOME:-$HOME/.local/state}/msfs-on-dgx-spark/tailscaled.state`), so successful login can be reused across runs. By default the script does not run interactive login automatically (`LOCAL_TAILSCALE_INTERACTIVE_LOGIN=0`) to avoid long hangs in headless runners; set `LOCAL_TAILSCALE_INTERACTIVE_LOGIN=1` to enable interactive login URL retrieval inside the script and tune `LOCAL_TAILSCALE_LOGIN_TIMEOUT_SECONDS` if needed. In interactive mode, the script now re-runs `tailscale up` automatically after login so successful auth can continue in the same run. When login is required, set `LOCAL_TAILSCALE_AUTH_URL_FILE=/path/to/url.txt` to persist the auth URL for external automation, and use `LOCAL_TAILSCALE_NEEDS_LOGIN_EXIT_CODE` (default `10`) to distinguish auth-required exits from generic failures.
When userspace bootstrap is script-started, cleanup now stops that daemon on exit by default (`LOCAL_TAILSCALE_CLEANUP_ON_EXIT=1`); set `LOCAL_TAILSCALE_CLEANUP_ON_EXIT=0` only when you intentionally want to keep it running for debugging.
When `BOOTSTRAP_LOCAL_TAILSCALE=1` is enabled, the script now auto-loads `LOCAL_TAILSCALE_AUTHKEY_FILE` from `$HOME/.config/msfs-on-dgx-spark/tailscale-authkey` when present (`AUTO_LOAD_LOCAL_TAILSCALE_AUTHKEY_FILE=1` by default), still enforcing mode `600` unless explicitly disabled.
...
When auth drift is expected, `AUTO_REAUTH_ON_AUTH_FAILURE=1` runs `58-ensure-steam-auth.sh` remotely before verification and also re-attempts auth between retry attempts after exit code `7` (unauthenticated). For unattended recovery provide `STEAM_USERNAME`/`STEAM_PASSWORD` (login form) and optionally `STEAM_GUARD_CODE` (2FA).
...
Launch flows now also guard against Steam's `game already running` state by default (`ENSURE_LAUNCHABLE_STATE=1` in `08-finalize-auth-and-run-msfs.sh` and `54-launch-and-capture-evidence.sh`). Before dispatch, the guard (`scripts/62-ensure-msfs-launchable-state.sh`) waits for existing wrapper sessions to settle, skips relaunch when MSFS is already active, and can auto-clean stale AppID wrappers + recover Steam runtime when wrappers are stuck.
...
Busy lock diagnostics now include lock-holder PID context when available (for example `holder pid: 12345`) and fallback lock mode can auto-reclaim stale lock directories by default (`MSFS_LOCK_RECLAIM_STALE=1`). For deterministic fallback-path testing/debugging, set `MSFS_FORCE_MKDIR_LOCK=1` to bypass `flock` and force mkdir lock mode. Both toggles are strict `0|1` values and now fail fast on invalid input. Lock names are also now constrained to `^[A-Za-z0-9_.-]+$` to prevent unsafe lock paths. Failed lock-acquire attempts now also clean partial lock state (globals/fd) before returning, so repeated retries in the same shell do not inherit stale lock context.
